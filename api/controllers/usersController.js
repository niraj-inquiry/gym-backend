    ...
    if (userdata.length > 0) {
      ...
      bcrypt.compare(password, userDetails.password, async (err, result) => {
        if (result == true) {
          ...
          const upres = await Users.updateOne(
            { _id: userDetails?._id },
            {
              $set: {
          ...
          if (upres?.acknowledged === true && upres?.modifiedCount > 0) {
            const token = jwt.sign({ id: userDetails?._id }, "secretkey", {
              expiresIn: "1d",
            });
...
sgMail.setApiKey(
  "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"
);
